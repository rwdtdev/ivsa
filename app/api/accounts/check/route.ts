...
import { VALID_TABEL_NUMBERS, makeResponseCORSLess, validateTabelNumber } from '@/lib/api/helpers';
import { cn } from '@/lib/api/ansi-helpers';
...
export async function GET(request: Request) {
  ...
  const tabelNumbers = Array.from(searchParams.entries()).filter(el => el[0]==='tabelNumber').map(el => el[1]);
  ...
  //    tabelNumber: true,
  //  },
  //})).map(el => el.tabelNumber);

  const usersFound = tabelNumbers.filter(el => VALID_TABEL_NUMBERS.includes(el) && !VALID_BUT_NON_EXISTANT_TABEL_NUMBERS.includes(el) );
