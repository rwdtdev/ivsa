...
export async function GET(request: NextRequest, context: iContext) {
  const CONSOLE_LOG_PREFIX = generateConsoleLogPrefix(request.method, '/api/inventories/{inventoryId}/events/{eventId}/link');
  ...
  clog(`inventoryId is ${isInventoryIdValid ? b('valid').gr() : b('invalid').rd()} (${cn(inventoryId)}), eventId is ${isEventIdValid ? b('valid').gr() : b('invalid').rd()} (${cn(eventId)})\nresponding with status ${resp.status}, '${resp.statusText}'`);
