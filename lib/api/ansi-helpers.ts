...
export const generateConsoleLogPrefix = (method: string, path: string) => {
  const methodColour = methodsColours.get(method) ?? '127;127;127';
  return `\x1B[33;1m[ \x1B[48;2;${methodColour};38;2;255;255;255m ${method} \x1B[49;33m \x1B[36;22;3m${path}\x1B[33;23;1m ]:\x1B[m `;
};

export function validityState<T>(
  paramName: string,
  // eslint-disable-next-line no-unused-vars
  validationCallback: (param: T) => boolean,
  param: T
): string {
...
export class ANSI {
  ...
  // Set colours:
  //   Foreground:
  rd = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;255;0;0m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  gr = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;0;255;0m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  bl = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;0;0;255m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  cn = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;0;255;255m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  mg = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;255;0;255m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  yl = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;255;255;0m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  wh = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;255;255;255m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  bla = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;0;0;0m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  gra = (background: boolean = false) =>
    new ANSI(
      `\x1B[${background ? '48' : '38'};2;127;127;127m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  //   Background:
  rdB = () => this.rd(true);
  grB = () => this.gr(true);
  ...
  col = (
    ...
    return new ANSI(
      `\x1B[${background ? '48' : '38'};2;${R};${G};${B}m${this._text}\x1B[${background ? '49' : '39'}m`
    );
  };
  colB = (r: string | number, g: string | number, b: string | number) =>
    this.col(r, g, b, true);
...
export const col = (
  ...
  g: string | number,
  b: string | number
) => new ANSI(text).col(r, g, b);
export const colB = (
  text: string,
  r: string | number,
  g: string | number,
  b: string | number
) => new ANSI(text).colB(r, g, b);
